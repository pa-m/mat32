...
func sameAnswerFloat(a, b interface{}) bool {
	if math32.IsNaN(a.(float32)) {
		return math32.IsNaN(b.(float32))
	}
...
func sameAnswerFloatApproxTol(tol float32) func(a, b interface{}) bool {
	return func(a, b interface{}) bool {
		if math32.IsNaN(a.(float32)) {
			return math32.IsNaN(b.(float32))
		}
		return EqualWithinAbsOrRel(a.(float32), b.(float32), tol, tol)
	}
}

func sameAnswerF32SliceOfSlice(a, b interface{}) bool {
	for i, v := range a.([][]float32) {
		if same := floatsSame(v, b.([][]float32)[i]); !same {
			return false
		}
	}
	return true
...
func makeRandOf(a Matrix, m, n int) Matrix {
	...
	case *VecDense:
		...
		mat := &VecDense{
			mat: blas32.Vector{
				Inc:  inc,
				Data: make([]float32, inc*(length-1)+1),
			},
			n: length,
		}
		for i := 0; i < length; i++ {
			mat.SetVec(i, randNormFloat32())
		}
		return mat
	case *basicVector:
		if m == 0 && n == 0 {
	...
	case *TriDense, *basicTriangular:
		...
		if triKind == Upper {
			for i := 0; i < m; i++ {
				for j := i; j < n; j++ {
					mat.SetTri(i, j, randNormFloat32())
				}
			}
		} else {
			for i := 0; i < m; i++ {
...
func makeCopyOf(a Matrix) Matrix {
	...
	case *VecDense:
		m := &VecDense{
			mat: blas32.Vector{
				Inc:  t.mat.Inc,
				Data: make([]float32, t.mat.Inc*(t.n-1)+1),
			},
			n: t.n,
		}
...
func equalApprox(a, b Matrix, tol float32, ignoreNaN bool) bool {
	ar, ac := a.Dims()
	br, bc := b.Dims()
	if ar != br {
		return false
	...
	for i := 0; i < ar; i++ {
		for j := 0; j < ac; j++ {
			if !EqualWithinAbsOrRel(a.At(i, j), b.At(i, j), tol, tol) {
				if ignoreNaN && math32.IsNaN(a.At(i, j)) && math32.IsNaN(b.At(i, j)) {
					continue
				}
				return false
			}
...
func underlyingData(a Matrix) []float32 {
	switch t := a.(type) {
	default:
...
var testMatrices = []Matrix{
	...
	NewVecDense(0, nil),
	&basicVector{},
	&VecDense{mat: blas32.Vector{Inc: 10}},
	&basicMatrix{},
	&basicTriangular{cap: 3, mat: blas32.Triangular{N: 3, Stride: 3, Uplo: blas.Upper}},
	&basicTriangular{cap: 3, mat: blas32.Triangular{N: 3, Stride: 3, Uplo: blas.Lower}},
	...
	Transpose{&VecDense{mat: blas32.Vector{Inc: 10}}},
	Transpose{&basicMatrix{}},
	Transpose{&basicTriangular{cap: 3, mat: blas32.Triangular{N: 3, Stride: 3, Uplo: blas.Upper}}},
	Transpose{&basicTriangular{cap: 3, mat: blas32.Triangular{N: 3, Stride: 3, Uplo: blas.Lower}}},
}
	...
	for _, aMat := range testMatrices {
		for _, bMat := range testMatrices {
			...
			for _, test := range sizePairs {
				...
				if !sameAnswer(want, got) {
					t.Errorf("Answer mismatch: %s want %v got %v", errStr, want, got)
				}
			}
		}
	}
		...
		for _, test := range sizes {
			...
			if aMaybeSame {
				...
				if panicked {
					...
					if !floatsEqual(preData, postData) {
