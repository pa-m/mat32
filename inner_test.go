...
func makeVecDenseInc(inc int, f []float32) *VecDense {
	v := &VecDense{
		n: len(f),
		mat: blas32.Vector{
			Inc:  inc,
			Data: make([]float32, (len(f)-1)*inc+1),
		},
	}
...
func benchmarkInner(b *testing.B, m, n int) {
	...
	randomSlice(data)
	mat := &Dense{mat: blas32.General{Rows: m, Cols: n, Stride: n, Data: data}, capRows: m, capCols: n}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		Inner(x, mat, y)
	}
